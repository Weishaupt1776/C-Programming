 ...
 * Brian W. Kernighan and Dennis M. Ritchie. Write a routine ungets(s)
 * that will push back an entire string onto the input. Should 
 * ungets() know about buf and bufp, or should it just use ungetch?
 */
  ...
  * Answer:
  * The function unget(s) should most definitely make use
  * of buf and bufp, but there does not have to be any explicit
  * mentioning of buf and bufp in the function ungets(s). The reason 
  * is because when ungets(s) is called, it takes an already
  * complete string and pushed it back to buf[] and stores it there
  * to be subsequently accessed as normal using getch(). */
	...
	while((type = getop(s)) != EOF)
	{
		switch(type)
		{
			...
			case '=':
				...
				break;
			case '<':
				printf("The last variable used was: %c = %f\n", recentVar, recentVal);
				break;
			default:
...
void ungets(const char *calculation)
{    
	int i = strlen(calculation);
	buf[bufp++] = '\n';
	
	while (i > 0)
	{
		ungetch(calculation[--i]);
	}
}
...
int getop(char s[])
{
	/* most recently stored character */
	int i, c;
	int testEOF;
	
	/* if buf[] is empty then get user input and store it all in buf[] */
	if(bufp == 0)
	{
		if((testEOF = gets(calculation)) == 0)
		{
			return EOF;
		}
		else
...
void clearStacks()
{
   /* clear the value stack by setting the pointer to the bottom, resets varsIndex counter */
   sp = 0;
   varsIndex = 0;
