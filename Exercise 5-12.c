 ...
 * Brian W. Kernighan and Dennis M. Ritchie. Extend entab and detab
 * to accept the shorthand "entab -m +n" to mean tab stops every n
 * columns, starting at column m. Choose convenient(for the user)
 */
...
int settab(int argc, char *argv[], char tabarr[])
{
	...
	int pos = 0, index = 0, inc = 0; /* index of tabarr*/
	...
	else if (argc == 3 && *argv[1] == '-' && *argv[2] == '+') 
	{
		...
		*p++;
		...
		/* error checking: of any invalid characters then end program */
		for (int i = 0; i <= 1; i++)
		{
			while (*p)
			{
        ...
        pos = atoi(&(*++argv)[1]); /* initial position of tab stop */
        inc = atoi(&(*++argv)[1]); /* tab stops at every increment */  
